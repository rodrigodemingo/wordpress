...
function get_icon_menu($meny){
    ...
    if (isset($meny->icon) && filter_var($meny->icon, FILTER_VALIDATE_URL)) {
        return "<img class='icon-header menu-icon nav-custom-icon' src='".$meny->icon."' alt='Menu icon'/>";
    } 
...
function render_megamenu_items($menies, $parent){
    ...
    foreach($menies as $meny){
        if( $meny->menu_item_parent == $parent ){
            ...
            if(trim($meny->image)!=''){
                $col_style = '<img src="'.trim($meny->image).'" class="menu-item-image" style="width:100%;" alt="Menu icon"/>';
            }
