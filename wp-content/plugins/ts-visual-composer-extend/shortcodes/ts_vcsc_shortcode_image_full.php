<?php
	...
	function TS_VCSC_Image_Full_Function ($atts) {
		...
		extract( shortcode_atts( array(
			...
			'slide_images'				=> '',
			'slide_mobile'				=> 'none', // none, show, first, random
			'slide_dynamic'				=> 'false',
			'slide_height'				=> 600,
			'slide_auto'				=> 'true',
			'slide_controls'			=> 'true',
		...
		if (($media_link_type != 'none') && ($type == "image")) {
			$link 						= TS_VCSC_Advancedlinks_GetLinkData($media_link_data);
			$a_href						= $link['url'];
			$a_title 					= $link['title'];
		...
		if ($type == "slideshow") {
			...
			$output .= '<div class="ts-slideshow-full-container" style="width: 100%; height: 100%; position: relative;">';				
				$output .= '<div id="' . $full_image_id . '" class="ts-slideshow-full-frame ' . $css_class . ' ' . $el_class . '" ' . $breakouts_data . ' style="width: 100%; height: 100%; margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px;" data-inline="' . $VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode . '" ' . $slider_settings . '>';
					...
					$image_random		= $slide_images[array_rand($slide_images)];
					$image_mobile		= '';
					$i					= 0;
					foreach ($slide_images as $single_image) {
						$i++;
						$slide_image	= wp_get_attachment_image_src($single_image, 'full');
						if (($i == 1) && ($slide_mobile == "first")) {
							$image_mobile = $slide_image;
						} else if (($single_image == $image_random) && ($slide_mobile == "random")) {
							$image_mobile = $slide_image;
						}
					...
					if (($slide_mobile == "first") || ($slide_mobile == "random")) {
						$output .= '<img class="ts-imagefull" src="' . $image_mobile[0] . '" data-width="' . $image_mobile[1] . '" data-height="' . $image_mobile[2] . '" data-no-lazy="1" alt="" style="width: 100%; height: auto; display: none;"/>';
					}
					$output .= $overlay_content;
					$output .= $raster_content;					
					...
					if (($slide_auto == 'true') && ($slide_controls == 'true')) {
						// Auto-Play Controls
