<?php
	...
	function TS_VCSC_Image_Switch_Function ($atts, $content = null) {
		...
		extract( shortcode_atts( array(
			...
			'switch_bar_shadow'				=> 'true',
			'switch_bar_type'				=> 'default', // default, custom
			'switch_bar_color'				=> '#0094FF',			
			'switch_click'					=> 'true',
			'switch_event'					=> 'none',
			'switch_link'					=> '',			
		...
		if ($switch_type == "ts-imageswitch-slide") {
			$switch_data					= 'data-event="' . $switch_event . '" data-handle="' . $slide_handle . '" data-start="' . $slide_start . '" data-direction="' . $slide_direction . '" data-trigger="' . $switch_trigger_slide . '"';
		...
		if ($attribute_alt_start == "true") {
			...
			$alt_attribute_start			= get_post_meta($image_start, '_wp_attachment_image_alt', true);
			if ($alt_attribute_start == "") {
		...
		if ($attribute_alt_end == "true") {
			...
			$alt_attribute_end				= get_post_meta($image_end, '_wp_attachment_image_alt', true);
			if ($alt_attribute_end == "") {
		...
		if (($switch_event == "link") && ($switch_link != '')){
			$link 							= TS_VCSC_Advancedlinks_GetLinkData($switch_link);
			$a_href							= $link['url'];
			$a_title 						= $link['title'];
			...
			$linkswitch_end					= '</a>';
			$linkswitch_modal				= '';
		} else if ($switch_event == "modal") {
			$linkswitch_start				= '<a id="' . $link_id . '" class="ts-imageswitch-link ' . $link_classes . ' nch-lightbox-modal nofancybox no-ajaxy" ' . $link_attributes . ' style="margin: 0; padding: 0;" href="#' . $switch_image_id . '-modal" target="_blank" title="' . $alt_attribute_start . '" data-type="html" ' . $nacho_data . ' data-random="' . $randomizer . '">';
			$linkswitch_end					= '</a>';
			// Create hidden DIV with Modal Content
			$linkswitch_modal = '<div id="' . $switch_image_id . '-modal" class="ts-modal-content nch-hide-if-javascript ' . $el_class . '" style="display: none; padding: 20px;">';
				$linkswitch_modal .= '<div class="ts-modal-white-header"></div>';
				$linkswitch_modal .= '<div class="ts-modal-white-frame" style="">';
					$linkswitch_modal .= '<div class="ts-modal-white-inner">';
						if (function_exists('wpb_js_remove_wpautop')){
							$linkswitch_modal .= wpb_js_remove_wpautop(do_shortcode($content), $wpautop);
						} else {
							$linkswitch_modal .= do_shortcode($content);
						}
					$linkswitch_modal .= '</div>';
				$linkswitch_modal .= '</div>';
			$linkswitch_modal .= '</div>';
		} else {
			$linkswitch_start				= '';
		...
		if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "false") {
			if ($image_responsive == "true") {
				$output .= $linkswitch_start;
					$output .= '<div id="' . $switch_image_id . '" data-event="' . $switch_event . '" data-trigger="' . $switch_trigger . '" class="' . $css_class . ' ' . $switch_class . '" ' . $switch_tooltipcontent . ' style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px; width: ' . $image_width_percent . '%;">';
						$output .= '<div id="' . $switch_image_id . '-counter" class="ts-switch-wrapper " style="width: ' . $image_width_percent . '%; ' . $image_height . '">';
							$output .= '<div style="' . $switch_padding . '" data-link="' . $link_id . '" ' . $switch_data . '>';
								...
								if ($switch_handle_show == "true") {
									if ($switch_type == "ts-imageswitch-slide") {
										$output .= '<div class="ts-imageswitch__handle ' . $switch_class . '" data-center="' . $switch_handle_center . '" style="' . $switch_handle_adjust . ' background-color: ' . $switch_bar_color . ';"><span class="frame_' . $switch_handle_icon . '" style="background-color: ' . $switch_handle_color . '"><i class="' . $switch_handle_icon . '"></i></span>';
									} else {
										$output .= '<div class="ts-imageswitch__handle ' . $switch_class . ' ' . ($switch_bar_shadow == "true" ? "ts-imageswitch-slide-shadow" : "") . '" style="' . $switch_handle_adjust . '"><span class="frame_' . $switch_handle_icon . '" style="background-color: ' . $switch_handle_color . '"><i class="' . $switch_handle_icon . '"></i></span>';
									}
									$output .= '</div>';
								} else if ($switch_type == "ts-imageswitch-slide") {
									$output .= '<div class="ts-imageswitch__handle ' . $switch_class . ' ' . ($switch_bar_shadow == "true" ? "ts-imageswitch-slide-shadow" : "") . '" data-center="' . $switch_handle_center . '" style="' . $switch_handle_adjust . ' background-color: ' . $switch_bar_color . ';"></div>';
								}
							$output .= '</div>';
						$output .= '</div>';				
					$output .= '</div>';
					$output .= $linkswitch_modal;
				$output .= $linkswitch_end;
			} else {
				$output .= $linkswitch_start;
					$output .= '<div id="' . $switch_image_id . '" data-event="' . $switch_event . '" data-trigger="' . $switch_trigger . '" class="' . $css_class . ' ' . $switch_class . '" ' . $switch_tooltipcontent . ' style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px; width: ' . $image_width . 'px;">';
