<?php
	...
	function TS_VCSC_Font_Icons_Function ($atts) {
		...
		if ($scroll_navigate == "gototop") {
			...
			if (($scroll_placement == "centerleft") || ($scroll_placement == "centerright")) {
				$margin_top				= -(($icon_size_slide / 2) + $icon_padding);
			}
			if (($scroll_placement == "topcenter") || ($scroll_placement == "bottomcenter")) {
				$gototop_style			.= " margin-left: -" . (($icon_size_slide / 2) + $icon_padding) . "px;";
			}
		} else {
			$gototop_class				= "";
			$gototop_data				= "";
