	...
	if (!class_exists('TS_Parameter_Gradient')) {
		class TS_Parameter_Gradient {
			...
			function gradient_settings_field($settings, $value) {
				...
				$border							= isset($settings['border']) ? $settings['border'] : 'false';
				...
				$html = '<div id="ts-advanced-gradient-' . $random_id_number . '" class="ts-advanced-gradient ts-settings-parameter-gradient-grey" data-identifier="' . $random_id_number . '" data-border="' . $border . '" data-trianglify="' . $trianglify . '" data-svg="' . $create_svg . '" data-default-color="' . $default_color . '" data-default-type="' . $default_type . '" data-default-shape="' . $default_shape . '">';
					...
					if ($trianglify == "false") {
						...
						$html .= '<div id="ts-advanced-gradient-custom-' . $random_id_number . '" class="ts-advanced-gradient-custom" style="display: none;">';
							...
							$html .= '<div class="ts-nouislider-input-slider" style="margin: 10px auto 10px auto; min-height: 35px;">';
								$html .= '<input id="ts-advanced-gradient-spread-' . $random_id_number . '" class="ts-advanced-gradient-spread" style="width: 100px; float: left; margin-left: 0px; margin-right: 10px;" name="" placeholder="0" data-identifier="' . $random_id_number . '" type="text" max="360" min="0" step="1" value="0"/>';
								$html .= '<span class="ts-nouislider-input-gradient-unit">deg</span>';
								$html .= '<span class="ts-nouislider-input-gradient-min">' . number_format_i18n(0, 0) . '&deg;</span>';
								$html .= '<span class="ts-nouislider-input-gradient-down dashicons-arrow-left"></span>';								
								$html .= '<div id="ts-nouislider-input-gradient-' . $random_id_number . '" class="ts-nouislider-input ts-nouislider-input-gradient" data-identifier="' . $random_id_number . '" data-value="0" data-min="0" data-max="360" data-decimals="0" data-step="1" style="width: 280px; float: left; margin-top: 10px;"></div>';
								$html .= '<span class="ts-nouislider-input-gradient-up dashicons-arrow-right"></span>';
								$html .= '<span class="ts-nouislider-input-gradient-max">' . number_format_i18n(360, 0) . '&deg;</span>';
							$html .= '</div>';
						$html .= '</div>';
