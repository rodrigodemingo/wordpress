<?php
									...
									while ($loop->have_posts()) : $loop->the_post();
										...
										if ($postCounter < $posts_limit + 1) {
											...
											$output .= '<li ' . $postAttributes . '>';
												...
												$output .= '</a></span>';
												// Product Price
												if (($ticker_price == "true") && ($variable == "false")) {
													...
													$output .= '</span>';								
												} else if (($ticker_price == "true") && ($variable == "true") && (count($prices) > 0)) {
													$output .= '<span class="ts-newsticker-pricetag" style="">';
														$output .= '<span class="ts-newsticker-product-current" style="float: none;">' . ((is_numeric(min($prices))) ? woocommerce_price(min($prices)) : min($prices)) . ' - ' . ((is_numeric(max($prices))) ? woocommerce_price(max($prices)) : max($prices)) . '</span>';
													$output .= '</span>';		
												}											
												// Sale Ribbon
												if ($onsale == "true") {
												...
												if (($ticker_cart == "true")) {
													if ($variable == "true") {
														$output .= '<a class="ts-newsticker-product-purchase" href="' . get_permalink() . '" rel="nofollow" data-id="' . $product_id . '" data-sku="' . $product_sku . '"><i style="" class="ts-newsticker-product-cart ts-woocommerce-product-cart ts-ecommerce-cart4"></i></a>';
													} else {
	...
	if ((class_exists('WPBakeryShortCode')) && (!class_exists('WPBakeryShortCode_TS_VCSC_WooCommerce_Ticker_Basic'))) {
