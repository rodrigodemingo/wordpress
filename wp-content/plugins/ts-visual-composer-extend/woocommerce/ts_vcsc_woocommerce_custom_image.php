<?php
	if (!class_exists('TS_WooCommerce_ImageGrid_Basic')){
		class TS_WooCommerce_ImageGrid_Basic {
								...
								if (($matched_terms == 0) && (($post_thumbnail != '') || ($data_grid_invalid == "false"))) {
									...
									if ($postCounter < $posts_limit + 1) {
										...
										if ($data_grid_price == "true") {
											if ($variable == "true") {
												$price				= strip_tags(((is_numeric(min($prices))) ? wc_price(min($prices)) : min($prices))) . ' - ' . strip_tags(((is_numeric(max($prices))) ? wc_price(max($prices)) : max($prices))) . ' / ';
											} else {
												$price				= $costs . ' / ';
											}
										} else {
			...
			// Add WooCommerce Basic Image Products Grid Elements
			function TS_VCSC_WooCommerce_ImageGrid_Basic_Elements() {
				global $VISUAL_COMPOSER_EXTENSIONS;
				// Add Basic Image Products Grid Element
	...
	if ((class_exists('WPBakeryShortCode')) && (!class_exists('WPBakeryShortCode_TS_VCSC_WooCommerce_ImageGrid_Basic'))) {
