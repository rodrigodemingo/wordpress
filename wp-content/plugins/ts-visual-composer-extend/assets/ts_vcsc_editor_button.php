<?php
	...
	function TS_VCSC_EditorButton($context) {
		if (!(strpos($_SERVER['REQUEST_URI'], 'widgets.php'))) {
			...
			if (get_option('ts_vcsc_extend_settings_useTinyMCEMedia', 1) == 0) {
				$context .= '<a id="ts_tiny_editor_button" class="button" title="' . $title . '" style="cursor: pointer; display: none;"><img src="' . $img . '" /> ' . __( "Add Icon", "ts_visual_composer_extend" ) . '</a>';
			} else {
				$context .= '<a id="ts_tiny_editor_button" class="button" title="' . $title . '" style="cursor: pointer;"><img src="' . $img . '" />' . __( "Add Icon", "ts_visual_composer_extend" ) . '</a>';
			}
			return $context;
		}
	}
						...
						<form id="ts_tiny_tinymce_form" name="ts_tiny_tinymce_form" autocomplete="off" style=""/>
							<table id="ts_tiny_tinymce_table_icon" class="ts_tiny_form ts_tiny_table ts_tiny_tinymce_table_icon" cellspacing="0" style="">
								<tr>
									...
									<td class="ts-switch-button ts-composer-switch">
										<div id="ts-tiny-tinymce-importer" class="toggle toggle-light" data-checkbox="ts-tiny-tinymce-importer-check" style="width: 110px; height: 20px;" data-on="<?php _e( "Show", "ts_visual_composer_extend" ); ?>" data-off="<?php _e( "Hide", "ts_visual_composer_extend" ); ?>">
