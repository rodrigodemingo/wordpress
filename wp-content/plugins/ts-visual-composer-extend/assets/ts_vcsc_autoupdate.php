<?php
	...
	class TS_VCSC_AutoUpdate {
		...
		public function __construct($current_version, $update_path = '', $plugin_slug = '', $license_user = '', $license_key = '', $multisite, $validator, $envatodata = '') {
			...
			if ($update_path == '') {
				$update_path						= 'http://maintenance.krautcoding.com/updates/ts-update-vc-extensions-wp.php';
			}
			// Redundancy Check
		...
		public function check_info($result, $action = null, $args = null) {
			...
			if ($information != false) {
				$plugin_data						= $VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_Envato_Globals;
				if (is_array($plugin_data)) {
					$plugin_rating					= isset($plugin_data["data"]["rating"]["rating"]) ? $plugin_data["data"]["rating"]["rating"] : 0;
					$plugin_votes					= isset($plugin_data["data"]["rating"]["count"]) ? $plugin_data["data"]["rating"]["count"] : 0;
					$plugin_sales					= isset($plugin_data["data"]["number_of_sales"]) ? $plugin_data["data"]["number_of_sales"] : 0;		
					if ($plugin_rating > 0) {
