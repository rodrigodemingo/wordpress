...
if(!function_exists('wpex_template_plugin')){
	function wpex_template_plugin($pageName){
		if (locate_template($pageName . '.php') != '') {
...
if(!function_exists('wpex_next_previous_timeline')){
	function wpex_next_previous_timeline($preevtrsl,$nextevtrsl){
			global $post;
			$args = array( 'post_type' => 'wp-timeline', 'posts_per_page' => 1, 'orderby'=> 'meta_value_num', 'order' => 'ASC','meta_key' => 'wpex_order', 'meta_value' => get_post_meta( $post->ID, 'wpex_order', true ),  'meta_compare' => '>', );
			...
			endforeach; 
			wp_reset_postdata();
			$args_pre = array( 'post_type' => 'wp-timeline', 'posts_per_page' => 1, 'orderby'=> 'meta_value_num', 'order' => 'DESC','meta_key' => 'wpex_order', 'meta_value' => get_post_meta( $post->ID, 'wpex_order', true ),  'meta_compare' => '<', );
